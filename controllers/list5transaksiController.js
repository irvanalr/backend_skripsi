...
const list5TransaksiModel = require("../models/list5transaksiModel");
...
const list5transaksi = (req, res) => {
    ...
    try {
      ...
      const decryptedHash = bytes.toString(CryptoJS.enc.Utf8);
       ...
       list5TransaksiModel.getTableToken((err, responseTableToken) => {
        if (err) {
        ...
        if (decryptedHash === originalTokenHash) {
          ...
          try {
            ...
            list5TransaksiModel.getTableOtentikasi((err, responseTableOtentikasi) => {
              if (err) {
                console.error('Error verifying token:', err);
                // Status code 500 Internal server error
                return res.status(500).json({
              ...
              if(originalTokenHash === decryptedHash) {
                ...
                list5TransaksiModel.getTableTransaksi(code, (err, responseTableTransaksi) => {
                  if (err) {
                    console.error('Error verifying token:', err);
                    // Status code 500 Internal server error
                    return res.status(500).json({
