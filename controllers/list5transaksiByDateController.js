...
const list5TransaksiByDateModel = require("../models/list5transaksiByDateModel");
...
const list5transaksiByDate = (req, res) => {
    // mendapatkan headers
    ...
    try {
      ...
      const decryptedHash = bytes.toString(CryptoJS.enc.Utf8);
       ...
       list5TransaksiByDateModel.getTableToken((err, responseTableToken) => {
        if (err) {
        ...
        if (decryptedHash === originalTokenHash) {
          ...
          try {
            ...
            list5TransaksiByDateModel.getTableOtentikasi((err, responseTableOtentikasi) => {
              if (err) {
                console.error('Error verifying token:', err);
                // Status code 500 Internal server error
                return res.status(500).json({
              ...
              if(originalTokenHash === decryptedHash) {
                list5TransaksiByDateModel.getTableTransaksi(code, date, (err, responseTableTransaksi) => {
                  if (err) {
                    console.error('Error verifying token:', err);
                    // Status code 500 Internal server error
                    return res.status(500).json({
