...
const informasiRekeningSimpananModel = require("../models/informasiRekeningSimpananModel");
...
const informasiRekeningSimpanan = (req, res) => {
    // mendapatkan headers
    ...
    try {
      ...
      const decryptedHash = bytes.toString(CryptoJS.enc.Utf8);
       ...
       informasiRekeningSimpananModel.getTableToken((err, responseTableToken) => {
        if (err) {
        ...
        if (decryptedHash === originalTokenHash) {
          ...
          try {
            ...
            informasiRekeningSimpananModel.getTableOtentikasi((err, responseTableOtentikasi) => {
              if (err) {
                console.error('Error verifying token:', err);
                // Status code 500 Internal server error
                return res.status(500).json({
              ...
              if(originalTokenHash === decryptedHash) {
                ...
                informasiRekeningSimpananModel.getAllTables(user.email, (err, responseAllTable) => {
                  ...
                  const filteredSaving = responseAllTable.map(saving => {
                    const { 
                      companyCode,
                      company,
                      branchCode,
                    ...
                    // Mengonversi UTC ke waktu Jakarta
                    const utcDate = new Date(registration);
                  ...
                  const coded = filteredSaving.find((user) => user.code === code);
                  
                  // Jika code pengguna tidak di temukan
                  if(!coded) {
                    // status code 404 not found
                    return res.status(404).json({
                      timestamp: formatDateToJakarta(new Date()),
