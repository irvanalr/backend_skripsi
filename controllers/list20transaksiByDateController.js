...
const list20transaksiByDateModel = require("../models/list20transaksiByDateModel");
...
const list20transaksiByDate = (req, res) => {
    // mendapatkan headers
    ...
    try {
      ...
      const decryptedHash = bytes.toString(CryptoJS.enc.Utf8);
       ...
       list20transaksiByDateModel.getTableToken((err, responseTableToken) => {
        if (err) {
        ...
        if (decryptedHash === originalTokenHash) {
          ...
          try {
            ...
            list20transaksiByDateModel.getTableOtentikasi((err, responseTableOtentikasi) => {
              if (err) {
                console.error('Error verifying token:', err);
                // Status code 500 Internal server error
                return res.status(500).json({
              ...
              if(originalTokenHash === decryptedHash) {
                list20transaksiByDateModel.getTableTransaksi(code, date, (err, responseTableTransaksi) => {
                  if (err) {
                    console.error('Error verifying token:', err);
                    // Status code 500 Internal server error
                    return res.status(500).json({
