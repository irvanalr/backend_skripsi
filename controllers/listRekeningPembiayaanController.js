...
const listRekeningPembiayaanModel = require("../models/listRekeningPembiayaanModel");
...
const listRekeningPembiayaan = (req, res) => {
    // Mendapatkan Headers
    ...
    try {
      ...
      const decryptedHash = bytes.toString(CryptoJS.enc.Utf8);
       ...
       listRekeningPembiayaanModel.getTableToken((err, responseTableToken) => {
        if (err) {
        ...
        if (decryptedHash === originalTokenHash) {
          ...
          try {
            ...
            listRekeningPembiayaanModel.getTableOtentikasi(namaPengguna, (err, responseTableOtentikasi) => {
              if (err) {
                console.error('Error verifying token:', err);
                // Status code 500 Internal server error
                return res.status(500).json({
              ...
              if(originalTokenHash === decryptedHash) {
                ...
                listRekeningPembiayaanModel.getAllTables(user.kode_cabang, (err, responseAllTable) => {
                  if (err) {
                    console.error('Error verifying token:', err);
                    // Status code 500 Internal server error
                    return res.status(500).json({
                  ...
                  // console.log(responseAllTable);

                  const filteredLoans = responseAllTable.map(saving => {
                    const { 
                      code, 
                      name, 
                      contract, 
                    ...
                    // Mengonversi UTC ke waktu Jakarta
                    const utcDate = new Date(contractDate);
